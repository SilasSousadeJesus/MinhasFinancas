...
namespace Minhas_Financas_Hangfire.Services
{
    public class BemPatrimonialJobs : IBemPatrimonialJobs
    {
        ...
        {
            var listaBemMaterial = await _context.Set<BemPatrimonial>().Include(p => p.DataPermanencia).ToListAsync();
            if (listaBemMaterial.Any()) {
                foreach (var bem in listaBemMaterial)
                {
                    if (!bem.DataPermanencia.Any()) continue;

                    var ultimaDataPermanencia = bem.DataPermanencia.OrderByDescending(x => x.DataPermanencia).ToList();
